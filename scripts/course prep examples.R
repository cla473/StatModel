...
ggplot(data1_trep, aes(transporter, rel_exp, colour=treatment)) + 
    geom_point()
...
ggplot(datab, aes(transporter, rel_exp, colour=treatment)) + 
    geom_point()
...
ggplot(turnip, aes(factor(density),  yield, colour=variety)) + 
    geom_point() +
    geom_boxplot() + 
...
ggplot(milk, aes(trt, yield, colour=trt)) +
    geom_boxplot() +
    scale_colour_manual(values=c("green","blue")) +
    labs(colour="Diet") +
    annotate("text", label="p=0.702", x=1.5, y=1000, color="black")
...
ggplot(milk, aes(Period, yield, colour=cow)) +
    geom_point() +
    geom_line() +
    facet_wrap(~group) +
...
ggplot(barley2, aes(Loc, Yield, colour=Var)) +
    geom_boxplot() +
...
ggplot(wheat, aes(Variety, Yield, colour=Variety)) +
    ...
    labs(y="Yield (tonnes/hectare)") + 
    theme(legend.position="top") +
    ylim(1,4)

wheat <- read.csv("raw_data/wheat yield.csv")
wheat$Variety <- relevel(wheat$Variety, ref="Standard")
str(wheat)
...
data2 <-read.csv("raw_data/Prac 3 mock LWR.csv")
...
cabbage <-cabbages
str(cabbage)
write.csv(cabbage,"raw_data/Prac 3 cabbage data.csv")
cabbage <- read.csv("raw_data/Prac 3 cabbage data.csv")
...
barley <-immer
write.csv(barley,"raw_data/Prac 3 barley yield.csv")
str(barley)
barley2 <- melt(barley, id.vars=c("Loc", "Var"), value.name="Yield", variable.name="Year")
str(barley2)
...
#setwd("C:/Users/u4321232/Dropbox/CSIRO short course/Data")
forest <- read.csv("raw_data/Prac 3 forest.csv")
str(forest)
...
ggplot(model0, aes(QuadDiam, Density, colour=StandType)) + 
    geom_point() +
    geom_hline(yintercept=3085) +
...
ggplot(model1, aes(QuadDiam, Density, colour=StandType)) +
    geom_point() +
    geom_abline(intercept=5153, slope=-162) +
    theme(legend.position="none")
...
#setwd("C:/Users/u4321232/Dropbox/CSIRO short course/Data")
photosynthesis <- read.csv("raw_data/Prac 4 photosynthesis.csv")
str(photosynthesis)
...
library(ggplot2)
#setwd("C:/Users/u4321232/Dropbox/CSIRO short course/Data")
nitro <- read.csv("raw_data/Prac 5 Arabidopsis nitrogen.csv")
str(nitro)
...
ggplot(nitro, aes(Day, log(PlantDM), colour=Nitrogen)) +
    geom_boxplot()
...
plant5$height <- 10 + as.numeric(plant5$Dose) + rnorm(36, 0, 2) + as.numeric(plant5$Row) - as.numeric(plant5$Column)

ggplot(plant5, aes(Dose, height)) +
    geom_point()
lmer2 <- lmer(height~Dose+(1|Row)+(1|Column), data=plant5)
anova(lmer2)
lm2 <- lm(height~Dose, data=plant5)
anova(lm2)
write.csv(plant5,"raw_data/Prac 5 plant growth.csv")

#Example 3: plant phenotype with nematodes

nem<-read.csv("raw_data/Prac 5 nematode.csv")
str(nem)
nem$Experiment <- factor(nem$Experiment)
ggplot(nem, aes(Genotype, Plant.weight, colour=Treatment)) +
    geom_boxplot()
ggplot(nem, aes(Genotype, lateral.root..root.weight, colour=Treatment)) +
    geom_boxplot()
